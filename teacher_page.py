    ...
    try:
        with sqlite3.connect(DB) as conn:
            ...
            if level_type == 'Upper Primary':
                ...
                rows = [dict(r) for r in conn.execute(query, (selected_class,)).fetchall()]
                if not rows: return None
                ...
                subject_keys = [
                    'literacy_i', 'literacy_ii', 'reading', 'luganda',
                    'mathematics', 'english', 'social_studies', 'science', 're_religious_education'
                ]
...
def edit_lower_record(record, on_save):
    with ui.dialog() as dialog, ui.card().classes('w-[700px] p-8'):
        ui.label(f"Edit Lower Primary Record: {record['pupil_name']}").classes('text-2xl font-bold mb-4')
        with ui.grid(columns=2).classes('w-full gap-4'):
            fields = {
                ...
                'science': ui.number('Science', value=record['science']),
                're_religious_education': ui.number('R.E', value=record['re_religious_education']),
                'class_teacher': ui.input('Teacher', value=record['class_teacher'])
            }
        
        def save():
            with sqlite3.connect(DB) as conn:
                conn.execute('''UPDATE lower_primary_results SET 
                    pupil_name=?, class_level=?, term=?, literacy_i=?, literacy_ii=?, 
                    reading=?, luganda=?, mathematics=?, english=?, social_studies=?, 
                    science=?, re_religious_education=?, class_teacher=? WHERE id=?''',
                    (fields['pupil_name'].value, fields['class_level'].value, fields['term'].value, 
                     fields['literacy_i'].value, fields['literacy_ii'].value, fields['reading'].value, 
                     fields['luganda'].value, fields['mathematics'].value, fields['english'].value, 
                     fields['social_studies'].value, fields['science'].value, fields['re_religious_education'].value, 
                     fields['class_teacher'].value, record['id']))
    ...
    def load_data():
        ...
        with content_area:
            try:
                ...
                if not raw_rows:
                    ui.label('No lower primary records found.').classes('text-gray-500 p-8 italic')
                else:
                    all_rows = compute_lower_derived_fields(raw_rows)
                    lower_table = ui.table(columns=columns, rows=all_rows, row_key='id').classes('w-full')
    ...
    def load_data():
        ...
        with content_area:
            try:
                with sqlite3.connect(DB) as conn:
                    conn.row_factory = sqlite3.Row
                    raw_rows = [dict(r) for r in conn.execute('SELECT * FROM academic_records').fetchall()]
                if not raw_rows:
...
def performance_analytics_hub_content():
    ...
    with content_container:
        ui.label('Per-Class Performance Hub').classes('text-2xl font-bold text-gray-800')
        ui.label('Select a section and class to view isolated performance metrics, top achievers, and intervention lists.').classes('text-sm text-gray-500 mb-2')
        
        # Fetch available classes dynamically from database
        try:
            with sqlite3.connect(DB) as conn:
                upper_classes = [r[0] for r in conn.execute('SELECT DISTINCT Class FROM academic_records WHERE Class IS NOT NULL').fetchall()]
                lower_classes = [r[0] for r in conn.execute('SELECT DISTINCT class_level FROM lower_primary_results WHERE class_level IS NOT NULL').fetchall()]
        except:
            upper_classes = []
            lower_classes = []
            ...
            with metrics_display_area:
                if not data:
                    with ui.card().classes('w-full p-8 items-center'):
                        ...
                        ui.label(f'No student records found for {class_name}.').classes('text-gray-500 mt-2')
                    return
                
                # KPI Summary Row
                with ui.row().classes('w-full gap-4'):
                    with ui.card().classes('flex-1 p-5 border-l-4 border-l-indigo-500'):
                        ui.label('Total Enrolled').classes('text-xs font-bold text-gray-400 uppercase')
                        ui.label(str(data['total_students'])).classes('text-2xl font-extrabold text-indigo-900')
                    
                    with ui.card().classes('flex-1 p-5 border-l-4 border-l-blue-500'):
                        ui.label('Class Average Score').classes('text-xs font-bold text-gray-400 uppercase')
                        ui.label(f"{data['class_avg']}%").classes('text-2xl font-extrabold text-blue-900')
                        
                    with ui.card().classes('flex-1 p-5 border-l-4 border-l-green-500'):
                        ui.label('Best Subject').classes('text-xs font-bold text-gray-400 uppercase')
                        ui.label(data['best_sub']).classes('text-lg font-bold text-green-800 truncate')
                        
                    with ui.card().classes('flex-1 p-5 border-l-4 border-l-red-500'):
                        ui.label('Weakest Subject').classes('text-xs font-bold text-gray-400 uppercase')
                        ui.label(data['worst_sub']).classes('text-lg font-bold text-red-800 truncate')
                
                # Detailed Breakdown Cards Row
                with ui.row().classes('w-full gap-6 mt-2'):
                    # Top Achievers
                    with ui.card().classes('flex-1 p-6 bg-green-50/50 border border-green-100'):
                        with ui.row().classes('items-center gap-2 mb-4'):
                            ui.icon('workspace_premium', color='green', size='1.5rem')
                            ui.label(f'Top Achievers — {class_name}').classes('text-md font-bold text-green-900')
                        
                        if not data['top_students']:
                            ui.label('No students recorded.').classes('text-xs text-gray-400 italic')
                        else:
                            for idx, s in enumerate(data['top_students'], 1):
                                ...
                                with ui.row().classes('w-full justify-between py-2 border-b border-green-100 items-center'):
                                    ui.label(f"{idx}. {name}").classes('font-medium text-sm text-gray-800')
                                    ui.label(f"{round(score, 1)}%").classes('font-mono font-bold text-sm text-green-700')

                    # Students Needing Intervention (< 50 average)
                    with ui.card().classes('flex-1 p-6 bg-red-50/50 border border-red-100'):
                        with ui.row().classes('items-center gap-2 mb-4'):
                            ui.icon('warning_amber', color='red', size='1.5rem')
                            ui.label(f'Academic Intervention — {class_name}').classes('text-md font-bold text-red-900')
                        
                        if not data['at_risk']:
                            ui.label('No students require urgent intervention.').classes('text-xs text-gray-400 italic')
                        else:
                            for s in data['at_risk']:
                                name = s.get('Name') or s.get('pupil_name') or 'Unknown'
                                score = s.get('Average') or s.get('_calc_avg') or 0
                                with ui.row().classes('w-full justify-between py-2 border-b border-red-100 items-center'):
                                    ui.label(name).classes('font-medium text-sm text-gray-800')
                                    ui.label(f"{round(score, 1)}%").classes('font-mono font-bold text-sm text-red-700')
...
def staff_chat_content():
    chat_container = ui.column().classes('w-full h-[450px] overflow-y-auto p-4 bg-slate-50 rounded-2xl shadow-inner border border-slate-200 gap-3')
    ...
    def load_messages():
        ...
        with chat_container:
            try:
                with sqlite3.connect(DB) as conn:
                    conn.row_factory = sqlite3.Row
                    rows = conn.execute('SELECT * FROM staff_chat ORDER BY id ASC').fetchall()
                
                if not rows:
                    with ui.column().classes('w-full items-center justify-center my-16 gap-2'):
                        ui.icon('local_cafe', size='2.5rem', color='amber-600')
                        ui.label('The kettle is boiling, but nobody has spoken yet!').classes('text-slate-600 text-xs font-semibold')
                        ui.label('Drop a complaint about the printer or start the morning gossip.').classes('text-slate-400 text-[11px] italic')
                else:
                    ...
                    for r in rows:
                        ...
                        bubble_bg = 'bg-[#800000] text-white rounded-tr-sm' if is_me else 'bg-white text-slate-800 rounded-tl-sm border border-slate-200 shadow-sm'
                        ...
                        with ui.column().classes(f'w-full {align_cls} gap-0.5 my-1'):
                            with ui.row().classes('items-center gap-2 px-1'):
                                ...
                                if is_me:
                                    ui.button(icon='delete_outline', on_click=lambda mid=msg_id: delete_message(mid)).props('flat dense').classes('text-red-400 hover:text-red-600 text-[10px] p-0 min-h-0 min-w-0')
                            
                            with ui.card().classes(f'{bubble_bg} p-3 rounded-2xl max-w-[75%] shadow-sm'):
                                ui.label(r['message']).classes('text-sm whitespace-pre-wrap leading-relaxed break-words')
                                with ui.row().classes('w-full justify-end items-center gap-1 mt-0.5'):
                                    ui.label(r['timestamp']).classes('text-[9px] text-amber-200' if is_me else 'text-slate-400')
                                    if is_me:
                                        ui.icon('done_all', size='12px', color='amber-300')
            except Exception as e:
                ui.label(f'⚠️ Printer jam in chat: {e}').classes('text-red-500 text-xs')

    with ui.column().classes('w-full max-w-3xl mx-auto bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden'):
        
        # Professional Maroon & Amber Header with Online Counter & Clear Feed Option
        with ui.row().classes('items-center justify-between w-full bg-[#800000] px-5 py-3.5 text-white border-b border-red-900'):
            with ui.row().classes('items-center gap-3'):
                with ui.avatar(size='sm').classes('bg-amber-500 text-[#800000] font-extrabold shadow-sm'):
                    ui.icon('school', size='1.1rem')
                with ui.column().classes('gap-0'):
                    ui.label('Staff Common Room').classes('text-sm font-extrabold text-white')
                    ui.label(current_vibe).classes('text-[11px] text-amber-300 font-medium')
            
            with ui.row().classes('items-center gap-1'):
                # Live online indicator badge
                ui.badge('Live Sync', color='amber-600').props('dense').classes('text-[10px] text-white font-bold')
                ui.timer(3.0, load_messages)
                ui.button(icon='refresh', on_click=load_messages).props('flat round dense').classes('text-white hover:text-amber-300')
        
        load_messages()
        
        # Fun Quick Reaction Emojis Bar
        quick_emojis = ["😂", "💀", "🔥", "👀", "☕", "📉", "💯", "🙏", "⚠️", "🤷‍♂️", "🤦‍♀️", "🍎", "📝", "📢"]
        with ui.row().classes('w-full gap-1 items-center px-3 py-1.5 bg-slate-50 border-t border-slate-200 overflow-x-auto'):
            ui.label('Vibe Check:').classes('text-[10px] text-amber-700 font-bold uppercase mr-1 shrink-0')
            for emo in quick_emojis:
                def insert_emoji(e=emo):
                    msg_input.value = (msg_input.value or '') + e
                    msg_input.update()
                ui.button(emo, on_click=insert_emoji).props('flat dense').classes('text-sm bg-white border border-slate-200 rounded-lg px-2 py-0.5 text-slate-700 hover:bg-amber-50 shrink-0 shadow-sm')
        ...
        with ui.row().classes('w-full items-center gap-2 p-3 bg-white border-t border-slate-200'):
            msg_input = ui.input(placeholder='Say something before the bell rings...').classes('flex-1 bg-slate-100 text-slate-900 rounded-xl px-3 border-none').props('borderless dense')
            ...
            def send_message():
                ...
                try:
                    with sqlite3.connect(DB) as conn:
                        conn.execute('INSERT INTO staff_chat (sender, message, timestamp) VALUES (?, ?, ?)', (sender, text, timestamp))
                    msg_input.value = ''
                    load_messages()
                except Exception as e:
            ...
            ui.button(icon='send', on_click=send_message).classes('bg-[#800000] text-white rounded-full p-2 h-10 w-10 shadow-sm hover:bg-red-900 transition-colors')
    ...
    with ui.column().classes('w-full items-center'):
        with ui.column().classes('w-[95%] max-w-7xl gap-8'):
            ...
            with ui.tabs().classes('w-full bg-white/80 backdrop-blur-md p-2 rounded-3xl shadow-xl border border-white/20') as tabs:
                ...
                ui.tab('Class Analytics', icon='analytics').classes('rounded-2xl transition-all duration-300 hover:bg-[#800000]/10')
                ui.tab('Upper Primary entry', icon='add_circle').classes('rounded-2xl transition-all duration-300 hover:bg-[#800000]/10')
                ui.tab('Upper Primary Records', icon='assignment').classes('rounded-2xl transition-all duration-300 hover:bg-[#800000]/10')
                ui.tab('Lower Primary entry', icon='add_circle').classes('rounded-2xl transition-all duration-300 hover:bg-[#800000]/10')
                ui.tab('lower Primary Records', icon='assignment').classes('rounded-2xl transition-all duration-300 hover:bg-[#800000]/10')
                ui.tab('Staff Chat', icon='forum').classes('rounded-2xl transition-all duration-300 hover:bg-[#800000]/10')
            ...
            with ui.tab_panels(tabs, value='Dashboard').classes('w-full bg-transparent'):
                ...
                with ui.tab_panel('Dashboard'):
                    ...
                    with ui.row().classes('w-full gap-4 mb-6'):
                        ...
                        with ui.card().classes('flex-1 p-5 border-l-4 border-l-[#800000] bg-white shadow-sm'):
                            ui.label('ACADEMIC CALENDAR').classes('text-xs font-bold text-gray-400 uppercase')
                            ui.label(f'{current_term}, {current_year}').classes('text-2xl font-extrabold text-[#800000]')
                            ui.label('Primary Section Active').classes('text-xs text-gray-500 mt-1')

                        # Total Enrolled Pupils Card (Automatic from Database)
                        try:
                            with sqlite3.connect(DB) as conn:
                                upper_count = conn.execute('SELECT COUNT(*) FROM academic_records').fetchone()[0]
                                lower_count = conn.execute('SELECT COUNT(*) FROM lower_primary_results').fetchone()[0]
                                total_pupils = upper_count + lower_count
                        except:
                            upper_count, lower_count, total_pupils = 0, 0, 0

                        with ui.card().classes('flex-1 p-5 border-l-4 border-l-blue-600 bg-white shadow-sm'):
                            ui.label('TOTAL ENROLLED').classes('text-xs font-bold text-gray-400 uppercase')
                            ui.label(str(total_pupils)).classes('text-2xl font-extrabold text-blue-900')
                            ui.label(f'Upper (P.4-P.7): {upper_count} | Lower (P.1-P.3): {lower_count}').classes('text-xs text-gray-500 mt-1')

                        # Candidate Class Focus (Automatic from Database)
                        try:
                            with sqlite3.connect(DB) as conn:
                                p7_count = conn.execute("SELECT COUNT(*) FROM academic_records WHERE Class LIKE '%7%'").fetchone()[0]
                        except:
                            p7_count = 0

                        with ui.card().classes('flex-1 p-5 border-l-4 border-l-amber-500 bg-white shadow-sm'):
                            ui.label('P.7 CANDIDATE CLASS').classes('text-xs font-bold text-gray-400 uppercase')
                            ui.label(str(p7_count)).classes('text-2xl font-extrabold text-amber-900')
                            ui.label('Registered for PLE Assessment').classes('text-xs text-gray-500 mt-1')
                ...
                with ui.tab_panel('Logout'):
                    with ui.card().classes('w-[300px] mx-auto items-center p-8'):
                        ...
                        def perform_logout():
                                    ...
                                    ui.navigate.to('/')
                        ui.button('Confirm Sign Out', color='red', on_click=perform_logout).classes('w-full')
