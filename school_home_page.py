            ...
            with ui.tab_panel(teachers_tab).classes('p-0 gap-4 flex flex-col'):
                with ui.card().classes('w-full p-6 bg-white shadow-md rounded-xl border border-slate-200'):
                    with ui.row().classes('w-full items-center justify-between'):
                        ui.label('Instructor Management Workspace').classes('text-lg font-bold text-slate-800')
                    ...
                    with ui.row().classes('w-full justify-between items-center'):
                        ...
                        with ui.row().classes('items-center gap-2'):
                            ...
                            def export_csv():
                                ...
                                df.write_csv(buffer)
                                ui.download(
                                    src=buffer.getvalue().encode('utf-8'), 
                            ...
                            ui.button('Refresh', icon='refresh', on_click=refresh_table_data).props('outline dense text-color="primary"')
                            ui.button('Print All Reports', icon='print', on_click=download_all_reports) \
